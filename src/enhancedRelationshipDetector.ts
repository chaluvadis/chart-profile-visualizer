      ...
      for (const target of resources) {
        if (
          ...
          const targetNamespace =
            target.namespace || target.metadata?.namespace;
          relationships.push({
    ...
    if (resource.kind === "Ingress") {
      ...
      for (const rule of rules) {
        ...
        for (const pathRule of paths) {
          const serviceName =
            pathRule.backend?.service?.name || pathRule.backend?.serviceName;
...
function detectStorageReferences(
  ...
  const volumes =
    resource.spec?.volumes || resource.spec?.template?.spec?.volumes || [];
...
function detectRBACReferences(
  ...
  const podSpec = resource.spec?.template?.spec || resource.spec;
  const saName =
    podSpec?.serviceAccountName || resource.spec?.serviceAccountName;
...
export function buildArchitectureNodes(
  ...
  const avgDegree =
    nodeArray.length > 0
      ? nodeArray.reduce((sum, n) => sum + n.inDegree + n.outDegree, 0) /
        nodeArray.length
      : 0;
