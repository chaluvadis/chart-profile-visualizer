...
exports.renderHelmTemplate = renderHelmTemplate;
exports.isHelmAvailable = isHelmAvailable;
const fs = __importStar(require("node:fs"));
const path = __importStar(require("node:path"));
const yaml = __importStar(require("js-yaml"));
const valuesMerger_1 = require("../processing/valuesMerger");
 ...
 */
async function renderHelmTemplate(chartPath, environment, releaseName = "release") {
    ...
    try {
        ...
        const { stdout, stderr } = await (0, cliRunner_1.runHelm)(args, {
            maxBuffer: constants_1.BUFFER_SIZE.HELM_OUTPUT, // 10MB buffer
            timeout: constants_1.TIMEOUT.HELM_TEMPLATE, // 30 second timeout
        });
        if (stderr) {
        ...
        else if (errorMessage.includes("missing required value") ||
 ...
 */
function parseHelmOutput(output, chartPath) {
    const resources = [];
...
async function isHelmAvailable() {
    try {
        await (0, cliRunner_1.runHelm)(["version", "--short"]);
        return true;
    }
    catch {
        return false;
 ...
 */
function getPlaceholderResources(chartPath, environment) {
    ...
    try {
        ...
        const comparison = (0, valuesMerger_1.mergeValues)(chartPath, environment);
        ...
        for (const templateFile of templateFiles) {
            ...
            try {
                ...
                const docs = [];
                yaml.loadAll(simplifiedYaml, (doc) => {
...
//# sourceMappingURL=helmRenderer.js.map