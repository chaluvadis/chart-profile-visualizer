...
export class HelmReleaseManager {
	 ...
	 * Get release history
	 */
	async getReleaseHistory(releaseName: string, namespace?: string): Promise<ReleaseHistoryEntry[]> {
		try {
			const cmd = this.buildHelmCommand(`history ${releaseName} -o json`, namespace);
			const { stdout } = await exec(cmd, { timeout: 10000 });
	...
	async getReleaseValues(
		...
		try {
			let cmd = this.buildHelmCommand(`get values ${releaseName} -o yaml`, namespace);
			if (revision) {
				cmd = this.buildHelmCommand(`get values ${releaseName} --revision ${revision} -o yaml`, namespace);
			}
			const { stdout } = await exec(cmd, { timeout: 10000 });
	 ...
	 */
	async getReleaseManifest(releaseName: string, namespace?: string, revision?: number): Promise<string> {
		try {
			let cmd = this.buildHelmCommand(`get manifest ${releaseName}`, namespace);
			if (revision) {
				cmd = this.buildHelmCommand(`get manifest ${releaseName} --revision ${revision}`, namespace);
			}
	 ...
	 * Get release notes
	 */
	async getReleaseNotes(releaseName: string, namespace?: string): Promise<string> {
		try {
	...
	async compareRevisions(
		...
		const valuesDiff = await this.generateDiff(yaml.dump(fromValues), yaml.dump(toValues));
	...
	async planUpgrade(
		...
		const currentManifest = await this.getReleaseManifest(releaseName, namespace);
		...
		for (const [id, newResource] of newResources) {
			...
			if (currentResource) {
				const breakingChanges = await this.detectBreakingChanges(currentResource, newResource);
				if (breakingChanges.length > 0) {
					plan.breakingChanges.push(...breakingChanges.map((c) => `${id}: ${c}`));
					plan.riskLevel = "high";
				}
			}
		}
		...
		try {
			...
			if (fs.existsSync(valuesYamlPath)) {
				const valuesYaml = yaml.load(fs.readFileSync(valuesYamlPath, "utf8")) as Record<string, unknown>;
		...
		if (plan.breakingChanges.length > 0) {
			...
			plan.recommendedActions.push("Consider running `helm upgrade --dry-run` first");
		}
	 ...
	 */
	async assessRollback(releaseName: string, targetRevision: number, namespace?: string): Promise<RollbackAssessment> {
		...
		if (!targetEntry) {
			assessment.canRollback = false;
			assessment.warnings.push(`Revision ${targetRevision} not found in history`);
			return assessment;
		}

		// Compare manifests
		const diff = await this.compareRevisions(releaseName, assessment.currentRevision, targetRevision, namespace);

		// Check for removed resources (data loss)
		for (const resourceId of diff.removedResources) {
			assessment.dataLossWarnings.push(`Resource will be removed: ${resourceId}`);
		...
		for (const change of diff.modifiedResources) {
			for (const fieldChange of change.changes) {
				if (fieldChange.isBreaking) {
					assessment.immutableFieldChanges.push(`${change.resourceId}: ${fieldChange.path}`);
		...
		if (assessment.immutableFieldChanges.length > 0) {
			assessment.recommendedActions.push("Some resources may need to be deleted and recreated");
		}
		assessment.recommendedActions.push("Run `helm rollback --dry-run` to preview changes");
	 ...
	 */
	private parseManifest(manifest: string): Map<string, Record<string, unknown>> {
	 ...
	 */
	private compareObjects(oldObj: unknown, newObj: unknown, path: string, changes: FieldChange[]): void {
	...
	private async detectBreakingChanges(
		...
		for (const field of immutableFields) {
			...
			if (oldValue && newValue && JSON.stringify(oldValue) !== JSON.stringify(newValue)) {
				changes.push(`${field.name} changed from ${JSON.stringify(oldValue)} to ${JSON.stringify(newValue)}`);
			}
		}

		// Check selector changes
		const oldSelector = this.getNestedValue(oldResource, "spec.selector.matchLabels");
		const newSelector = this.getNestedValue(newResource, "spec.selector.matchLabels");

		if (oldSelector && newSelector) {
			for (const [key, value] of Object.entries(oldSelector as Record<string, unknown>)) {
	 ...
	 */
	private findRequiredValues(values: Record<string, unknown>, prefix = ""): string[] {
		...
		for (const [key, value] of Object.entries(values)) {
			...
			if (value === null || value === "") {
				...
				required.push(...this.findRequiredValues(value as Record<string, unknown>, path));
			}
		}
