...
function initTopology() {
    ...
    const NODE_WIDTH = Math.max(MIN_NODE_WIDTH, Math.min(MAX_NODE_WIDTH, LEFT_RESERVED + maxNameLength * CHAR_WIDTH_APPROX + RIGHT_RESERVED));
 ...
 */
function applyTierFilter(container, filterTier) {
 ...
 */
function renderTiers(container, activeTiers, width, height, margin, tierHeight, startY, NODE_WIDTH, nodePositions) {
 ...
 */
function renderEdges(container, edges, nodePositions, NODE_WIDTH) {
    edges.forEach(edge => {
 ...
 */
function renderNodes(container, nodes, edges, nodePositions, kindIcons, NODE_WIDTH) {
 ...
 */
function createCriticalBadge(indicatorX) {
 ...
 */
function createConnectivityBadge(indicatorX, connY, totalConnections) {
 ...
 */
function handleNodeClick(container, nodes, edges, nodeId, nodeElement) {
    ...
    edges.filter(e => e.source === nodeId || e.target === nodeId).forEach(edge => {
 ...
 */
function initPanAndZoom(svg, container) {
    let isPanning = false;
 ...
 */
function updateTopologyZoom(container) {
    if (container) {
