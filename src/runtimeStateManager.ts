...
export class RuntimeStateManager {
  ...
  private onDidChangeState: vscode.EventEmitter<string> =
  ...
  async getChartRuntimeState(
    ...
    const state = await this.fetchChartRuntimeState(
      chartPath,
      environment,
      namespace,
    );
  ...
  private async fetchChartRuntimeState(
    ...
    try {
      const renderedResources = await renderHelmTemplate(
        chartPath,
        environment,
      );
      ...
      for (const resource of renderedResources) {
        ...
        const resourceNamespace =
          resource.namespace || namespace || clusterInfo.namespace || "default";
        ...
        try {
          ...
          warnings.push(
            `Could not get state for ${resource.kind}/${resource.name}: ${errorMessage}`,
          );
        }
      }
    } catch (error: unknown) {
  ...
  async getResourceEvents(
    ...
    Array<{ lastSeen: string; type: string; reason: string; message: string }>
  > {
