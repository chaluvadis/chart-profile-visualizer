...
import {
  generateDependencyVisualizationData,
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const visualizeChartCommand = vscode.commands.registerCommand(
    ...
    async (item: unknown) => {
      await showChartVisualization(
        context,
        item as Parameters<typeof showChartVisualization>[1],
      );
    },
  );
  ...
  const checkClusterStatusCommand = vscode.commands.registerCommand(
    "chartProfiles.checkClusterStatus",
    async () => {
      ...
      if (!clusterInfo.connected) {
        vscode.window.showErrorMessage(
          `Not connected to cluster: ${clusterInfo.errorMessage}`,
        );
        return;
      }
    ...
    async (item: unknown) => {
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          lines.push(
            `- **Overall Status**: ${healthSummary.overallStatus.toUpperCase()}`,
          );
  ...
  const planUpgradeCommand = vscode.commands.registerCommand(
    "chartProfiles.planUpgrade",
    async (item: unknown) => {
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          ...
          const plan = await releaseManager.planUpgrade(
            typedItem.name || "release",
            chartPath,
          );
  ...
  const assessRollbackCommand = vscode.commands.registerCommand(
    ...
    async (item: unknown) => {
      ...
      if (!releaseName) {
        ...
        const releaseManager = getHelmReleaseManager();
        const releases = await releaseManager.listReleases(
          typedItem?.namespace,
        );
      ...
      await vscode.window.withProgress(
        ...
        async () => {
          const releaseManager = getHelmReleaseManager();
          const history = await releaseManager.getReleaseHistory(
            releaseName || "",
            typedItem?.namespace,
          );
          ...
          if (history.length < 2) {
            vscode.window.showErrorMessage(
              "Need at least 2 revisions to assess rollback",
            );
            return;
          }
          ...
          lines.push(
            `- **Can Rollback**: ${assessment.canRollback ? "Yes" : "No"}`,
          );
    ...
    async (item: unknown) => {
      ...
      if (viz.nodes.length > 1) {
        ...
        for (const node of viz.nodes) {
          ...
          lines.push(
            `${indent}- ${status} **${node.label}** (${node.version})`,
          );
          if (node.repository) {
