...
import {
  ChartProfilesProvider,
  ChartTreeItem,
} from "./core/chartProfilesProvider";
import { show as showChartVisualization } from "./visualization/chartVisualizationView";
import {
  compareEnvironments,
  formatComparisonForWebview,
  type ComparisonWebviewData,
} from "./diff/environmentDiff";
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const compareEnvironmentsCommand = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      const charts = await import("./k8s/helmChart").then((m) =>
      ...
      try {
        ...
        try {
          ...
          console.error(
            "Failed to format comparison for webview:",
            formatError,
          );
